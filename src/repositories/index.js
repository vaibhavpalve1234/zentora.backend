...
class UserRepository extends BaseRepository {
  ...
  async updateLastLogin(id, { requestId } = {}) {
    return query(`UPDATE users SET updated_at = NOW() WHERE id = ?`, [id], { requestId });
  }
}
...
class InvestmentRepository extends BaseRepository {
  ...
  async findByUser(userId, { type, status, requestId } = {}) {
    ...
    return query(`SELECT * FROM investments WHERE ${clauses.join(' AND ')} ORDER BY created_at DESC`, params, { requestId });
  }
  ...
  async getPortfolioSummary(userId, { requestId } = {}) {
    const sql = `
      SELECT
        ...
        ROUND((SUM(current_value) - SUM(invested_amount)) * 100.0 / NULLIF(SUM(invested_amount), 0), 2) AS pnl_percentage
      FROM investments
  ...
  async getTotalPortfolioValue(userId, { requestId } = {}) {
    const [row] = await query(
      `SELECT SUM(invested_amount) AS total_invested, SUM(current_value) AS total_current_value
       FROM investments WHERE user_id = ? AND status IN ('active','paused')`,
      [userId],
      { requestId }
    );
    return row;
  ...
  async getSIPList(userId, { requestId } = {}) {
    return query(
      `SELECT * FROM investments WHERE user_id = ? AND type = 'sip' AND status = 'active' ORDER BY sip_amount DESC`,
      [userId],
      { requestId }
    );
  }
...
class LoanRepository extends BaseRepository {
  constructor() {
    super('loans', [
      'id','user_id','direction','party_name','party_name_bi','party_contact','principal',
  ...
  async findByUser(userId, { direction, status, requestId } = {}) {
    ...
    return query(`SELECT * FROM loans WHERE ${clauses.join(' AND ')} ORDER BY due_date ASC`, params, { requestId });
  }

  async getLoanSummary(userId, { requestId } = {}) {
    const sql = `
      SELECT direction, COUNT(*) AS count, SUM(principal) AS total_principal, SUM(outstanding) AS total_outstanding
      FROM loans
  ...
  async getOverdueLoans(userId, { requestId } = {}) {
    return query(
      `SELECT * FROM loans WHERE user_id = ? AND status IN ('active','partially_paid') AND due_date < CURDATE() ORDER BY due_date ASC`,
      [userId],
      { requestId }
    );
  }
  ...
  async getDueInDays(userId, days = 7, { requestId } = {}) {
    return query(
      `SELECT * FROM loans WHERE user_id = ? AND status IN ('active','partially_paid') AND due_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL ? DAY) ORDER BY due_date ASC`,
      [userId, days],
      { requestId }
    );
  }
  ...
  async recordPayment(loanId, paymentData, { requestId } = {}) {
    return this.transaction(async (conn) => {
      const [ins] = await conn.execute(
        `INSERT INTO loan_payments (id, loan_id, user_id, amount, payment_date, principal_component, interest_component, notes)
         VALUES (UUID(), ?, ?, ?, ?, ?, ?, ?)`,
        [
          loanId,
          paymentData.user_id,
      ...
      await conn.execute(
        `UPDATE loans
         SET outstanding = GREATEST(0, outstanding - ?),
             status = CASE WHEN outstanding - ? <= 0 THEN 'paid' ELSE status END
         WHERE id = ?`,
  ...
  async findActive(userId, { requestId } = {}) {
    return query(
      `SELECT b.*, c.name AS category_name, c.icon AS category_icon
       ...
       WHERE b.user_id = ? AND b.is_active = 1 AND (b.end_date IS NULL OR b.end_date >= CURDATE())
        ...
        ROUND(COALESCE((
          ...
          WHERE t.user_id = b.user_id
            ...
            AND t.transaction_date >= CASE b.period
              ...
              ELSE CURDATE()
            END
        ), 0) * 100.0 / NULLIF(b.amount, 0), 2) AS utilization_pct
      FROM budgets b
      LEFT JOIN categories c ON c.id = b.category_id
...
class InsightRepository extends BaseRepository {
  ...
  async findByUser(userId, { insightType, unreadOnly, limit = 10, requestId } = {}) {
    ...
    return query(
      `SELECT * FROM ai_insights
       WHERE ${clauses.join(' AND ')} AND (expires_at IS NULL OR expires_at > NOW())
  ...
  async existsByContextHash(userId, hash, { requestId } = {}) {
    const [row] = await query(
      `SELECT id FROM ai_insights WHERE user_id = ? AND context_hash = ? AND (expires_at IS NULL OR expires_at > NOW()) LIMIT 1`,
      [userId, hash],
      { requestId }
    );
    return !!row;
